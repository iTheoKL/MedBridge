...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Storage
  // ---------------------------------------------------------------------
  // Default reminders, generated from the same medication list shown on
  // the Medications page, so a first-time visitor sees a consistent,
  // populated schedule instead of an empty page. Only used the very
  // first time (i.e. when nothing has been saved to storage yet) — any
  // edits after that are the user's own and take over from here on.
  function buildDefaultReminders() {
    const today = todayISO();

    const seed = (name, dosage, time, frequency, customFrequency, mealTiming, priority, notes) => ({
      id: uid(),
      name,
      dosage,
      time,
    ...
    return [
      seed("Metformin", "500mg", "08:00", "twice-daily", "", "after", "normal", "With breakfast."),
      seed("Metformin", "500mg", "20:00", "twice-daily", "", "after", "normal", "With dinner."),
      seed("Ibuprofen", "200mg", "13:00", "custom", "As needed", "after", "normal", "Only take when needed for pain."),
      seed("Warfarin", "5mg", "20:00", "daily", "", "with", "critical", "Avoid taking with NSAIDs — bleeding risk."),
      seed("Paracetamol", "500mg", "08:00", "custom", "Three times daily", "after", "normal", "Take after meals for fever or pain relief."),
      seed("Paracetamol", "500mg", "14:00", "custom", "Three times daily", "after", "normal", "Take after meals for fever or pain relief."),
      seed("Paracetamol", "500mg", "20:00", "custom", "Three times daily", "after", "normal", "Take after meals for fever or pain relief."),
      seed("Aspirin", "400mg", "09:00", "twice-daily", "", "with", "important", "Take with food — increases bleeding risk when combined with Warfarin."),
      seed("Aspirin", "400mg", "21:00", "twice-daily", "", "with", "important", "Take with food — increases bleeding risk when combined with Warfarin."),
    ];
  }
